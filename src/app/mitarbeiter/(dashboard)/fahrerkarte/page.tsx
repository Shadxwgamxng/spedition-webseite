...
async function callCard(action: string, employeeId: string, extra?: Record<string, unknown>) {
...
function OwnCard({
  ...
  async function run(action: string) {
    setBusy(true);
    try {
      await callCard(action, employeeId);
      await refetch();
    } finally {
      setBusy(false);
    }
      ...
      {unread.map((reminder) => (
        <div key={reminder.id} className="flex items-start gap-3 rounded-2xl border border-amber-400/40 bg-amber-400/10 p-4">
          ...
          <button
            type="button"
            disabled={busy}
            onClick={() => callCard("ack-reminder", employeeId, { reminderId: reminder.id }).then(refetch)}
...
function AllCards({ cards, refetch }: { cards: DriverCardRecord[]; refetch: () => Promise<void> }) {
  ...
  async function sendReminder(employeeId: string) {
    setSending(true);
    try {
      await callCard("remind", employeeId, { text: reminderText });
      await refetch();
      setReminderFor(null);
    } finally {
      setSending(false);
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setReminderFor(reminderFor === card.employeeId ? null : card.employeeId)}
                ...
                <div className="mt-4 space-y-2 rounded-xl bg-mist-100 p-3">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <button
                      type="button"
                      disabled={sending}
                      onClick={() => sendReminder(card.employeeId)}
